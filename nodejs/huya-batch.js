...
const fireFetch = async (url, defaultRes = {}, isJson = false) => {
    ...
    try {
        const res = await fetch(url).then(res => isJson ? res.json() : res.text())
        //  console.log(res);
        return res
    } catch (e) {
        console.error(e)
...
const getYqkRooms = async () => {
    ...
    const genUrl = (tmpId) => `https://live.cdn.huya.com/livelist/game/tagLivelist?gameId=2135&tmpId=${tmpId}&callback=getLiveListJsonpCallback&page=1`
    const rooms = [];
    for (const tmpId of tmpIds) {
        console.log(`获取一起看 ${tmpId} 子分区  的房间列表`);
        let resTxt = await fireFetch(genUrl(tmpId), { data: { datas: [] } }, false)
        resTxt = resTxt.replace('getLiveListJsonpCallback(', '').replace('}})', '}}'), res = JSON.parse(resTxt)
        const { data, status } = res;
        if (status === 200) {
            const list = data.datas || [];
            const rs = list.map(({ profileRoom, introduction, nick, uid }) => ({ roomid: profileRoom, introduction, nick, uid }))
            rooms.push(...rs)
        }
...
(async () => {
    const jsonList = [], rooms = [... await getYqkRooms()]
