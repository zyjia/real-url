const {
    isJSONValid, COMM_CONF,
    fireFetch, encrypt,
    genUrlSearch, matchHtmlText
...
const getRoomRealId = async (rid) => {
    ...
    const html = await fireFetch(url) || "", text = matchHtmlText(html, /{"rid":\d{1,8},"vipId":0/) + '}'
    if (isJSONValid(text)) {
    ...
    ub9FuncStr = ub9FuncStr.replace('(function (', 'function sign(').replace('CryptoJS.MD5(cb).toString()', `"${rb}"`).replace('rt;})', 'rt;};')
    const signFuncStr = new VMScript(`${ub9FuncStr} sign("${info.rid}","${info.did}","${info.t10}");`)
...
async function getRoomPreviewInfo(rid) {
    ...
    const realId = initInfo.rid + '', url = 'https://playweb.douyucdn.cn/lapi/live/hlsH5Preview/' + realId,
        body = {
            rid: realId,
            did: initInfo.did,
        }, auth = encrypt(realId + initInfo.t13), headers = {
            rid: realId,
            time: initInfo.t13,
            auth,
    ...
    const res = await fireFetch(url, {method: 'post', headers, body: genUrlSearch(body, true)}, true)
    let error = res['error'],
        data = res['data'],
        key = ''
    if (data) {
...
const getRoomLiveUrls = async (rid) => {
    ...
    if (prevInfo.key) {
        ...
        real_url["flv"] = `http://vplay1a.douyucdn.cn/live/${prevInfo.key}.flv?uuid=`
        real_url["x-p2p"] = `http://vplay1a.douyucdn.cn/live/${prevInfo.key}.xs?uuid=`

    }
    return real_url
}

/*getRoomLiveUrls(7570958).then(res => {
    console.log(res)
})*/
//批量解析房间
